...
def home_view(request): 
    ...
    user_posts_model =Posts.objects.all()
    ...
    context = {'user_profile' : user_profile,'post':post,'user_post':feed_lists,'suggestions_username_profile_list': suggestions_username_profile_list[:4],
               'user_posts_model':user_posts_model}
...
def upload_view(request):
    if request.method == 'POST':
        ...
        texts = request.POST['caption']
        images = request.FILES.get('post_photo')
        videos = request.FILES.get('post_video')
        files = request.FILES.get('post_file')
        audios = request.FILES.get('post_audio')
        
        new_posts =Posts.objects.create(user=users,image=images,text=texts,audio=audios,video=videos,File=files)
        new_posts.save()
